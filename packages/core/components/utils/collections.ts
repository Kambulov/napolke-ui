...
export const isNapolkeElement = (el?: HTMLElement): boolean => {
  if (!el) return false
  if (el?.dataset && el?.dataset['napolke']) return true
  el.attributes.getNamedItem('data-napolke')
  return !!el.attributes.getNamedItem('data-napolke')
}
