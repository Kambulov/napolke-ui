import { NapolkeUIThemes } from './presets'
import type { DeepPartial } from '../utils/types'
import lightTheme from './presets/default'
import darkTheme from './presets/dark'

export type NapolkeUserTheme = DeepPartial<NapolkeUIThemes> & { type: string }
...
const getPresets = (): Array<NapolkeUIThemes> => {
  return [lightTheme, darkTheme]
}

const getPresetStaticTheme = (): NapolkeUIThemes => {
  return lightTheme
}
...
const isPresetTheme = (
  themeOrType?: NapolkeUserTheme | NapolkeUIThemes | string,
): boolean => {
...
const hasUserCustomTheme = (themes: Array<NapolkeUIThemes> = []): boolean => {
  return !!themes.find(item => isAvailableThemeType(item.type))
}

const create = (base: NapolkeUIThemes, custom: NapolkeUserTheme): NapolkeUIThemes => {
  if (!isAvailableThemeType(custom.type)) {
    throw new Error('Duplicate or unavailable theme type')
  }

  return deepDuplicable(base, custom) as NapolkeUIThemes
}

const createFromDark = (custom: NapolkeUserTheme) => create(darkTheme, custom)
const createFromLight = (custom: NapolkeUserTheme) => create(lightTheme, custom)
