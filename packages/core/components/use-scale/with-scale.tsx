    ...
    return (
      <ScaleContext.Provider value={value}>
        {typeof Render === 'string' ? (
          // Если Render - это строка (имя HTML-элемента), то передаем null
          <>{children}</>
        ) : (
          // Иначе, проверяем, поддерживает ли компонент свойство ref
          React.isValidElement(Render) && Render.hasOwnProperty('ref') ? (
            // Если поддерживает, передаем ref
